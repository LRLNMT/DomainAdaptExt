...
generate () {
    ...
    domain=$1
    model_path=$2
    # path_2_model="./${model_path}/checkpoints/checkpoint_last.pt"
    ...
    do
        ...
        sacrebleu -tok spm -s none "../Datasets/${lang}/test/${test_set}/test-${lang_tag_to}.txt" < "${save_test_path}.hyp" -m chrf bleu > "${save_dir_path}/scores.json" 
...
main () {
    ...
    stage1_save_path="mixed-${domain1}-${size1}_${lang_pair}"
    ...
    # perl -E "print '=' x 80 "
    # printf "\n"
    # echo "******************** Start STAGE 1 Training ${lang_pair} using ${domain1}-${size1} ********************\n"
    # train $domain1 "${PRETRAINED_PATH}/model.pt" "./${save_path}/stage1_${domain1}_model"
    # echo "******************** Finished STAGE 1 Training ${lang_pair} using ${domain1}-${size1} ********************\n"
    ...
    perl -E "print '=' x 80 "
    printf "\n"
    echo "******************** Start STAGE 2 Training ${lang_pair} using ${domain2}-${size2} ********************\n"
    # train $domain2 "./${stage1_save_path}/stage1_${domain1}_model/checkpoints/checkpoint_last.pt" "./${save_path}/stage2_${domain1}-${domain2}_model"
    train $domain2 "./${stage1_save_path}/stage1_${domain1}_model/checkpoints/checkpoint_best.pt" "./${save_path}/stage2_${domain1}-${domain2}_best_model"
    echo "******************** Finished STAGE 2 Training ${lang_pair} using ${domain2}-${size2} ********************\n"

    # STAGE 2 Testing
    echo "******************** Start STAGE 2 Generation ${lang_pair} ********************\n"
    generate $domain2 "./${save_path}/stage2_${domain1}-${domain2}_best_model"
...
main $language $reverse $dom1 $dom1_size $dom2 $dom2_size
